...
На данный момент присутствует весь основной функционал кроме выбора нескольких параметров в фильтрах (только по одному).
Так же еще не причесан код и не присутствуют комментарии. Это все доделаю и добавлю завтра. Есть баги, которые так же планирую устранить.
Если возник вопрос, зачем в таком виде загружать?)))) Просто хочу показать прогресс, ну и плюс, если что откатиться до рабочей версии. 
