...
>В [репозитории находится api-сервер](https://github.com/SeregaSE/adgo-test/tree/master/server), который умеет общаться по `HTTP`. Данное api предоставит вам данные которые необходимы для выполнения данного задания. [Подробное описание api-сервера](#api) в конце readme.
...
Сделайте `fork` данного репозитория. Реализуйте свое решение в директории [app](tree/master/app). Когда будете готовы, создайте `pull request` в `master` ветку нашего репозитория. Заодно мы оценим ваше умение работать с `GIT`.
