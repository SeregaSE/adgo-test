## Тестовое задание на должность Junior Frontend разработчик в AdGo

### Задача
Наша компания занимается рекламой и мы собираем очень очень очень много статистических данных о показах, кликах и количестве денег которое заработали со всего этого дела. Одним из основных инструментов нашей системы является интерфейс для просмотра и анализа подбного рода данных. Мы называем его "статистика" и предлагаем реализовать Вам упрощенную и легкую версию данного инструмента.
...
Мы хотим чтобы Вы реализовали вывод подобных статистических данных в таблицу c возможностью сгруппировать, отфильтровать набор данных, а также возможность постраничной навигации.

![scheme](docs/scheme.png "Scheme")
<center><i>Схематичное представление задания.</i></center>

В данном репозитории лежит api-сервер, который умеет общаться по `HTTP` и отдавать статистику о которой писали выше. (Смотри раздел с API)

Для запуска веб сервера выполните команду `npm start`

И еще раз расскажем о задании более детально:
- Почитать и разобраться в задаче;
- Убедитесь, что в вашей системе установлен node.js последней версии;
- Разобраться с API;
- Нужно сделать фильтр с выбором:
	- Периода за который происходит выбор статистики;
	- Поля по которому будет происходить группировка данных;
	- Выбор платформы, браузеров и операционных систем, о которых будет отображена информация.
- Нужно сделать табличку которая покажет данные;
- Нужно сделать постраничную навигацию, например по 20 строчек на странице.
...
Фронтэнд должен быть написан на **React** и пожалуйста, приложите инструкцию как нам запустить Ваше творение в пару простых команд.

Мы не хотим накладывать на вас ограничения, вы можете использовать кроме **React** любые другие библиотеки на свой выбор, но нам в свою очередь хочеться увидеть **ваш код** и **ваши решения**.

Про css и какие-то билиотеки готовых компонентов, например **Bootstrap**. Вы можете оформить проект как угодно, даже если это будет чистый `html` с минимальным количетсвом `css`, нам хватит. Если вы не можете не делать супер красивые интерфейсы и любите какой-то набор компонентов, можете воспользоваться им).
